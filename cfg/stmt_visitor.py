...
uninspectable_modules = {module.name for module in iter_modules()}
...
class StmtVisitor(ast.NodeVisitor):
    ...
    def visit_Try(self, node):
        ...
        for handler in node.handlers:
            ...
            handler_body = self.handle_stmt_star_ignore_node(
                handler_body, handler_node)
    ...
    def assign_tuple_target(self, target_nodes, value_nodes, right_hand_side_variables):
        ...
        return ControlFlowNode(new_assignment_nodes[0], [new_assignment_nodes[-1]], [])
    ...
    def add_blackbox_or_builtin_call(self, node, blackbox):  # noqa: C901
        ...
        for arg_node in itertools.chain(node.args, node.keywords):
            arg = arg_node.value if isinstance(
                arg_node, ast.keyword) else arg_node
    ...
    def add_module(  # noqa: C901
        ...
        new_module_definitions = ModuleDefinitions(
            local_names, module_or_package_name)
        ...
        self.local_modules = get_directory_modules(
            module_path) if self._allow_local_modules else []
        ...
        self.visit(tree)
        exit_node = self.append_node(
            EntryOrExitNode('Module Exit ' + str(module[0])))
        ...
        for real_name in real_names:
            ...
            if os.path.isdir(full_name):
                ...
                if os.path.isfile(new_init_file_location):
                    ...
                    raise Exception(
                        'from anything import directory needs an __init__.py file in directory')
            else:
    ...
    def handle_relative_import(self, node):
        ...
        if node.level == 1:
            ...
            if node.module:
                name_with_dir = os.path.join(
                    no_file, node.module.replace('.', '/'))
    ...
    def visit_Import(self, node):
        ...
        for alias in node.names:
            ...
            if alias.name not in uninspectable_modules:
                ...
                uninspectable_modules.add(alias.name)
        return IgnoredNode()
    ...
    def visit_ImportFrom(self, node):
        ...
        for name in node.names:
            local_definitions = self.module_definitions_stack[-1]
            local_definitions.import_alias_mapping[name.asname or name.name] = "{}.{}".format(
